    ...
    <>
      <aside className={`h-screen ${expanded ? "w-60" : "w-12"} sticky top-0 left-0`}>
        <nav className="h-full flex flex-col bg-white border-r shadow-sm">
          ...
          <NavLink to={"/profile"} onClick={() => setExpanded(false)} title="Profile" className={`h-16 flex ${expanded ? "justify-start px-3" : "justify-center"} items-center gap-x-3 border-b-[1px]`}>
